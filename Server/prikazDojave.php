	...
	<div id="wrapper">
		...
		<?php
			...
			function ispis(){
				...
				$query = 'SELECT * FROM dojave ORDER BY dojava_primljena DESC LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
				$sqli = mysqli_query($connect,$query);
				
				echo "
					<form id='form_pretraga' action='pretragaDojava.php' method='get'>
						Pretraži prema: 
						<select name='odabir_pretrage' id='select_pretraga'>
							<option value='ime'>Ime</option>
							<option value='prezime'>Prezime</option>
							<option value='vrsta_nesrece'>Vrsta nesreće</option>
							<option value='podvrsta_nesrece'>Podvrsta nesreće</option>
							<option value='ozlijedeni'>Ozlijeđeni</option>
							<option value='vozila'>Vozila</option>						
						</select>
						<input type='text' id='pretraga'  name='kljuc_pretrage' placeholder='Unesite ključ pretrage... '/>
						<input type='submit' value='Pretraži' id='trazi'>
					</form>
				";
				...
				for ($page=1;$page<=$number_of_pages;$page++) {
				  echo '<a class="broj_stranice" href="prikazDojave.php?page=' . $page . '">' . $page . '</a> ';
				}
				echo "</div>";
			}
		?>
